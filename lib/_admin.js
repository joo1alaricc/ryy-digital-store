...
export function createAdminToken(admin) {
  ...
  return sign({
    ...
    iat: now,
    // Admin sessions are intentionally non-expiring. Access remains protected by the signed token and admin credentials.
    exp: 0
  });
}
...
export function verifyAdminToken(token) {
  try {
    ...
    // exp=0 is the permanent admin-session marker. Older tokens with a finite exp still work until their original expiry.
