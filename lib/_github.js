...
export function runWithRequestCache(fn) { return requestStore.run({ databasePromise: null }, fn); }
function requestCache() { return requestStore.getStore(); }
...
export async function readDatabase() {
  ...
  if (cache?.databasePromise) return cache.databasePromise;
  const load = async () => {
    const kv = getKV();
    if (kv) {
      const cached = await kv.get(DATABASE_KV_KEY, "json");
      if (cached && typeof cached === "object") return { database: cached, sha: null, storage: "kv" };
      let seed = null; let seedSha = null;
      try { const gh = await readDatabaseFromGitHub(); seed = gh.database; seedSha = gh.sha; }
      catch (error) { console.warn("GitHub seed unavailable, trying static database asset:", error?.message || error); }
      if (!seed) seed = await readDatabaseFromAsset();
      if (!seed) seed = cloneInitialDatabase();
      if (!seed || typeof seed !== "object") throw new Error("Database awal tidak tersedia.");
    ...
    try { return await readDatabaseFromGitHub(); }
    catch (error) {
  ...
  const pending = load();
  if (cache) cache.databasePromise = pending;
  try { return await pending; } catch (error) { if (cache) cache.databasePromise = null; throw error; }
}

export async function writeDatabase(database, sha, message) {
  const cache = requestCache();
  if (cache) cache.databasePromise = null;
  const kv = getKV();
