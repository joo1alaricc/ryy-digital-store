 ...
 * 2) On first use, KV is seeded from GitHub when configured, otherwise from
 *    the deployed static data/database.json.
...
export async function readDatabase() {
  ...
  if (kv) {
    ...
    if (!seed || typeof seed !== "object") {
      throw new Error("Database belum tersedia. Pastikan data/database.json ada di deployment atau konfigurasi GitHub benar.");
    }

    await kv.put(DATABASE_KV_KEY, JSON.stringify(seed));
    return { database: seed, sha: seedSha, storage: "kv" };
  }

  // Backward-compatible fallback for deployments where STORE_KV is not bound.
  // A static asset is enough for read-only/login flows; write flows still need
  // either STORE_KV or a working GitHub configuration.
  try {
    return await readDatabaseFromGitHub();
  } catch (error) {
    console.warn("GitHub database unavailable, trying static database asset:", error?.message || error);
    const assetDatabase = await readDatabaseFromAsset();
    if (!assetDatabase) throw error;
    return { database: assetDatabase, sha: null, storage: "asset" };
  }
}
...
export async function databaseStorageStatus() {
  ...
  const config = githubConfig();
  return { primary: "github", initialized: config.missing.length === 0, missing: config.missing };
}
