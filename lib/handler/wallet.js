...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='cancelDeposit'){
      const orderId=String(req.body?.orderId||'').trim();
      const dep=w.deposits.find(d=>String(d.orderId||'')===orderId);
      if(!dep)return res.status(404).json({success:false,message:'Deposit tidak ditemukan.'});
      if(dep.status==='completed')return res.status(409).json({success:false,message:'Deposit sudah berhasil. Deposit yang sudah selesai tidak dapat dibatalkan.'});
      if(dep.status!=='pending')return res.status(409).json({success:false,message:'Deposit ini sudah dibatalkan atau berakhir.'});
      dep.status='cancel'; dep.cancelledAt=new Date().toISOString(); dep.updatedAt=dep.cancelledAt; dep.cancelledBy='user';
      addWalletTransaction(user,{type:'deposit_cancelled',idr:Number(dep.amount||0),coins:Number(dep.coins||0),reason:'Deposit QRIS dibatalkan user',orderId:dep.orderId,transactionId:dep.transactionId||''});
      await writeDatabase(database,sha,`Cancel BTZ deposit ${dep.orderId} by ${user.username}`);
      return res.status(200).json({success:true,message:'Deposit berhasil dibatalkan.',deposit:dep,wallet:safeWallet(user),user:safeUser(user)});
    }
