...
export default async function handler(req,res){
  try{
    ...
    if(req.method==="GET"){
      ...
      const unreadByUser=c.messages.filter(m=>m.sender==="admin"&&!m.readByUser).length;
      // Badge polling uses peek=1 and must not mark messages as read.
      if(String(req.query?.peek||"") === "1") {
        return res.status(200).json({success:true,conversation:{email:c.email,username:c.username,updatedAt:c.updatedAt||"",messages:[],unreadByUser}});
      }
      ...
      return res.status(200).json({success:true,conversation:c,unreadByUser:0});
    }
