...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Google already verifies the identity and the email address. Do not add a
    // second OTP/WhatsApp verification step for Google sign-in.
    user.googlePendingVerification = false;
    ...
    return res.status(200).json({ success:true, verificationRequired:false, email, user:safeUser(user), token:createUserToken(user.id) });
  } catch (error) {
