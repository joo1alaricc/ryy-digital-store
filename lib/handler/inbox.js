...
import { notifyAdminOrderImage } from "../gateway-registry.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    } else if (action === "submitImage") {
      const messageId=String(req.body?.messageId||""); const purchaseId=String(req.body?.purchaseId||"");
      const imageUrl=String(req.body?.imageUrl||"").trim();
      const message=user.inbox.find(m=>String(m.id)===messageId);
      if(!message || message.type!=="imageUpload") return res.status(404).json({success:false,message:"Permintaan upload tidak ditemukan."});
      if(message.responded) return res.status(409).json({success:false,message:"Gambar sudah pernah dikirim."});
      if(!isValidUploadedImageUrl(imageUrl)) return res.status(400).json({success:false,message:"Gambar tidak valid."});
      const purchase=user.pendingPurchases.find(p=>p.id===purchaseId); if(!purchase) return res.status(404).json({success:false,message:"Pesanan tidak ditemukan."});
      purchase.loginScreenshotUrl=imageUrl; purchase.loginScreenshotAt=new Date().toISOString();
      message.responded=true; message.read=true; message.imageUrl=imageUrl;
      const processedBy={username:purchase.processedBy||"admin_utama",email:purchase.processedByEmail||""};
      notifyAdminOrderImage({admin:processedBy,purchaseId,imageUrl,caption:`– SCREENSHOT LOGIN USER\n· ID Pesanan: ${purchaseId}\n· User: ${user.username}`}).catch(e=>console.error("WA screenshot:",e));
    } else {
