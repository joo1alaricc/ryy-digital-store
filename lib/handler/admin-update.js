...
function makeDiff(pkg, repo) {
  ...
  for (const path of repo.files.keys()) {
    // Backup artifacts are intentionally ignored during update diffing.
    // They may exist in the GitHub repo but are not part of the deploy ZIP,
    // so their absence must never be reported as a deletion.
    if (path.toLowerCase().endsWith(".bak")) continue;
...
const UPDATE_BATCH_SIZE = 35;
const UPDATE_JOB_PREFIX = "ryy:update-job:v2:";
const UPDATE_JOB_TTL = 3600;

function updateKV() {
  return binding("STORE_KV");
}

async function saveUpdateJob(id, state) {
  const store = updateKV();
  if (!store?.put) fail("STORE_KV belum tersedia untuk melanjutkan proses update.", 503);
  await store.put(`${UPDATE_JOB_PREFIX}${id}`, JSON.stringify(state), { expirationTtl: UPDATE_JOB_TTL });
}

async function loadUpdateJob(id) {
  const store = updateKV();
  if (!store?.get) fail("STORE_KV belum tersedia untuk melanjutkan proses update.", 503);
  const state = await store.get(`${UPDATE_JOB_PREFIX}${id}`, "json");
  if (!state) fail("Sesi update sudah kedaluwarsa. Jalankan Validasi Update lagi.", 409);
  return state;
}

async function deleteUpdateJob(id) {
  const store = updateKV();
  if (store?.delete) await store.delete(`${UPDATE_JOB_PREFIX}${id}`);
}

function changedPackageEntries(pkg, diff) {
  const changedSet = new Set(diff.added.concat(diff.changed));
  return pkg.entries
    .filter(e => e.path !== "index.html" && changedSet.has(e.path))
    .sort((a, b) => a.path.localeCompare(b.path));
}

async function createBlob(entry, cfg) {
...
async function processUpdateBatch(pkg, state) {
  const diff = state.diff;
  const cfg = githubConfig();
  const entries = changedPackageEntries(pkg, diff);
  const indexEntry = pkg.map.get("index.html");

  if (entries.length !== state.totalEntries) {
    fail("Isi ZIP berubah sejak validasi. Jalankan Validasi Update lagi.", 409);
  }

  let nextIndex = Number(state.nextIndex || 0);
  const end = Math.min(entries.length, nextIndex + UPDATE_BATCH_SIZE);
  const treeEntries = Array.isArray(state.treeEntries) ? state.treeEntries.slice() : [];

  for (let i = nextIndex; i < end; i++) {
    treeEntries.push(await createBlob(entries[i], cfg));
  }
  nextIndex = end;

  // Keep index.html for the final batch so the application file becomes the
  // last blob prepared before the atomic Git commit.
  if (nextIndex >= entries.length && indexEntry && diff.added.concat(diff.changed).includes("index.html") && !state.indexPrepared) {
    treeEntries.push(await createBlob(indexEntry, cfg));
    state.indexPrepared = true;
  }
  ...
  if (nextIndex < entries.length || (indexEntry && diff.added.concat(diff.changed).includes("index.html") && !state.indexPrepared)) {
    return { done: false, state, progress: nextIndex, total: entries.length };
  }

  for (const path of diff.deleted) treeEntries.push({ path, mode: "100644", type: "blob", sha: null });

  if (!treeEntries.length) {
    return { done: true, result: { diff, committed: false, message: "Tidak ada perubahan yang perlu diterapkan." } };
  }
  ...
  const commit = await github(`/repos/${cfg.owner}/${cfg.repo}/git/commits`, {
    method: "POST",
    body: JSON.stringify({
      message: `RYY STORE update by ${state.adminUsername} — ${diff.changed.length} changed, ${diff.added.length} added, ${diff.deleted.length} deleted`,
      tree: tree.sha,
      parents: [state.commitSha]
    })
  });
  await github(`/repos/${cfg.owner}/${cfg.repo}/git/refs/heads/${encodeURIComponent(state.branch)}`, {
    method: "PATCH",
  ...
  return {
    ...
    result: {
      ...
      commitUrl: commit.html_url || `https://github.com/${cfg.owner}/${cfg.repo}/commit/${commit.sha}`,
      message: "Update berhasil diterapkan ke GitHub dalam satu commit atomic."
    }
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action !== "apply" && action !== "continue") return res.status(400).json({ success: false, message: "Action update tidak dikenal." });
    if (!expected) return res.status(409).json({ success: false, message: "Validasi update kedaluwarsa atau file ZIP berubah. Jalankan validasi ulang sebelum menerapkan." });
    if (expected !== pkg.manifestHash) return res.status(409).json({ success: false, message: "Validasi update kedaluwarsa atau file ZIP berubah. Jalankan validasi ulang sebelum menerapkan." });

    let state;
    let updateId = String(body.get("updateId") || "");
    if (action === "continue") {
      if (!updateId) return res.status(400).json({ success: false, message: "ID proses update tidak ditemukan." });
      state = await loadUpdateJob(updateId);
      if (state.manifestHash !== pkg.manifestHash || state.adminUsername !== admin.username) {
        return res.status(409).json({ success: false, message: "Sesi update tidak cocok dengan ZIP atau admin yang aktif." });
      }
    } else {
      const repo = await getRepoSnapshot();
      const diff = makeDiff(pkg, repo);
      ...
      state = {
        ...
        diff,
        totalEntries: changedPackageEntries(pkg, diff).length,
        nextIndex: 0,
        treeEntries: [],
        indexPrepared: false
      };
    ...
    const batch = await processUpdateBatch(pkg, state);
    if (!batch.done) {
      await saveUpdateJob(updateId, batch.state);
      return res.status(200).json({
        ...
        total: batch.total,
        diff: state.diff,
        message: `Menyiapkan file update ${batch.progress}/${batch.total}...`
      });
    }

    await deleteUpdateJob(updateId);
    return res.status(200).json({ success: true, action, continue: false, updateId, manifestHash: pkg.manifestHash, ...batch.result });
  } catch (error) {
