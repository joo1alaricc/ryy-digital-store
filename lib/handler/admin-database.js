...
const ENV_KEYS = [
  ...
  "TURNSTILE_SECRET_KEY","TURNSTILE_SITE_KEY","GEMINI_API_KEY","GEMINI_MODEL",
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==="env-save"){
      ...
      return res.status(200).json({success:true,message:req.body?.clear===true?`${key} dikembalikan ke environment Cloudflare.`:`${key} berhasil disimpan sebagai override aplikasi.`,value: req.body?.clear===true ? env(key,"") : value,override:req.body?.clear!==true});
    }
    if(action==="env-reset-all"){
      const s=store();
      if(!s) return res.status(503).json({success:false,message:"STORE_KV binding tidak tersedia."});
      await s.put(ENV_OVERRIDES_KEY,JSON.stringify({}));
      return res.status(200).json({success:true,message:"Semua override ENV dihapus. Aplikasi kembali memakai environment Cloudflare."});
    }
    if(action==="export"){
      const {database}=await readDatabase();
