...
import { ensureWallet, debitCoins, refundCoins, starterOfferEligible, markStarterOfferUsed, unmarkStarterOffer } from "../_finance.js";
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'cancelPurchase') {
            ...
            if (purchase.paymentMethod === 'RYY_COIN' && Number(purchase.totalCoins || 0) > 0) {
                ...
                for (const item of (purchase.items || [])) {
                    if (item.starterOffer) unmarkStarterOffer(user, item.productName, item.typeName, purchase.id);
                }
            }
        ...
        const activeGatewayPurchase = user.pendingPurchases.find(p =>
            p?.status === 'pending' &&
            p?.gatewayPayment &&
            String(p.paymentStatus || '') === 'pending' &&
            p.gatewayPayment.transactionId
        );
        if (activeGatewayPurchase) {
            return res.status(409).json({
                success:false,
                code:'PENDING_QRIS_CHECKOUT',
                message:'Selesaikan dulu pembayaran kamu.',
                pendingPurchase:activeGatewayPurchase
            });
        }
        ...
        if (!items.length || !paymentMethod || (!coinPayment && !gatewayQris && !proofUrl)) {
            return res.status(400).json({ success:false, message:coinPayment || gatewayQris ? "Produk dan metode pembayaran wajib diisi." : "Produk, metode pembayaran, dan bukti pembayaran wajib diisi." });
        }
