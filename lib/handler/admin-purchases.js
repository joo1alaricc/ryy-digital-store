...
import { notifyPrimaryApproval, isPrimaryAdmin } from "../gateway-registry.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!purchaseId || !userId || !["confirm","reject","requestForm","requestLoginScreenshot"].includes(action)) return res.status(400).json({success:false,message:"Data tindakan pesanan tidak lengkap."});
    ...
    if(action==="requestLoginScreenshot"){
      const messageId=`msg_${crypto.randomUUID()}`;
      user.inbox.unshift({id:messageId,type:"imageUpload",read:false,createdAt:new Date().toISOString(),title:"Screenshot halaman login diperlukan",body:"Silahkan screenshot halaman login, lalu upload gambarnya pada blok upload di bawah pesan ini.",purchaseId:pending.id,responded:false});
      pending.loginScreenshotRequestAt=new Date().toISOString(); pending.loginScreenshotRequestedBy=admin.username;
      await writeDatabase(database,databaseSha,`Request login screenshot ${purchaseId}`);
      return res.status(200).json({success:true,message:"Permintaan screenshot halaman login dikirim ke inbox user.",user:safeUser(user)});
    }
    ...
    pending.status="confirmed";pending.processedAt=new Date().toISOString();pending.processedBy=admin.username;pending.processedByEmail=admin.email||"";pending.confirmedTotalItems=totalItems;pending.confirmedTotalSpent=totalSpent;pending.adminNote=note;pending.formCompletedAt=form.required.length?new Date().toISOString():pending.formCompletedAt||"";pending.delivery={status:"pending",email:user.email||"",sentAt:""};
    const productData=String(req.body?.productData||"").trim().slice(0,10000);
    ...
    // Non-primary approvals are audited to the two notification inboxes and the primary WhatsApp gateway.
    if(!isPrimaryAdmin(admin)){
      const approvalHtml=`<div style="font-family:Arial,sans-serif;line-height:1.6"><h2>RYY STORE</h2><p>Pesanan <b>${htmlEscape(purchaseId)}</b> telah di-ACC oleh <b>${htmlEscape(admin.username)}</b> (${htmlEscape(admin.email||"-")}).</p></div>`;
      await Promise.allSettled([
        sendEmail({to:"rizaladitia992@gmail.com",subject:`Pesanan di-ACC — ${purchaseId}`,html:approvalHtml}),
        sendEmail({to:"eilyansander@gmail.com",subject:`Pesanan di-ACC — ${purchaseId}`,html:approvalHtml}),
        notifyPrimaryApproval({purchaseId,admin})
      ]);
    }
