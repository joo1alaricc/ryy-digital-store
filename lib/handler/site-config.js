...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      uiMode,
      otherTools: getOtherTools().filter(tool=>{ const found=Array.isArray(config?.otherTools)?config.otherTools.find(x=>String(x?.id)===tool.id):null; return found ? found.enabled===true : tool.defaultEnabled===true; }).map(x=>({id:x.id,name:x.name,description:x.description})),
