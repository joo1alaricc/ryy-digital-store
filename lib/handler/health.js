...
import { readAdminConfigKV, getAdminUsers, getEnvAdminFallback } from "../_admin.js";
...
export default async function handler(req, res) {
  ...
  const result = {
    ...
    build: "cloudflare-admin-v4-kv-2026-08-22",
  ...
  result.config = {
    ...
    ADMIN_SUPER_USERS: Boolean(env("ADMIN_SUPER_USERS"))
  };
  ...
  try {
    ...
    const adminKv = await readAdminConfigKV();
    const envAdminCount = getAdminUsers().length + (getEnvAdminFallback() ? 1 : 0);
    result.admin = {
      storage: adminKv.available ? "kv" : "env/github",
      initialized: adminKv.available ? adminKv.initialized : envAdminCount > 0,
      persistentAdminCount: adminKv.admins.length,
      envAdminCount,
      loginReady: Boolean(result.config.APP_SECRET && (adminKv.admins.length > 0 || envAdminCount > 0))
    };
    result.auth = {
      ...
      adminLoginReady: Boolean(result.config.APP_SECRET && (result.admin?.persistentAdminCount > 0 || result.admin?.envAdminCount > 0))
    };
  } catch (error) {
    result.database = { ok: false, message: error?.message || "Database status failed." };
    const adminKv = await readAdminConfigKV().catch(() => ({ available: false, initialized: false, admins: [] }));
    const envAdminCount = getAdminUsers().length + (getEnvAdminFallback() ? 1 : 0);
    result.admin = { storage: adminKv.available ? "kv" : "env/github", initialized: adminKv.available ? adminKv.initialized : envAdminCount > 0, persistentAdminCount: adminKv.admins.length, envAdminCount, loginReady: Boolean(result.config.APP_SECRET && (adminKv.admins.length > 0 || envAdminCount > 0)) };
    result.auth = { loginReady: false, googleReady: false, otpConfigReady: false, adminLoginReady: Boolean(result.config.APP_SECRET && (adminKv.admins.length > 0 || envAdminCount > 0)) };
  }
