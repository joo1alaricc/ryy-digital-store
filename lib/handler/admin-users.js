...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "PATCH") {
      ...
      if (body.username !== undefined || body.email !== undefined || body.secondaryContact !== undefined || body.avatar !== undefined) {
        return res.status(400).json({ success:false, message:"User ID, username, email, kontak tambahan, dan foto profil tidak dapat diedit admin." });
      }
