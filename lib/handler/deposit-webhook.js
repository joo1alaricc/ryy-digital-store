  ...
  try{
    ...
    if(!foundUser||!dep){
      ...
      if(purchaseHit){
        ...
        if(purchase.status==='cancelled' || purchase.paymentStatus==='cancel') return res.status(200).json({success:true,message:'Checkout sudah dibatalkan user. Pembayaran tidak dikreditkan.'});
    ...
    if(dep.status==='cancel')return res.status(200).json({success:true,message:'Deposit sudah dibatalkan user. Pembayaran tidak dikreditkan.'});
