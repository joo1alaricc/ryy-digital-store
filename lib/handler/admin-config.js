...
import { verifyAdminToken, isMainAdmin, readAdminConfigKV, writeAdminConfigKV, normalizeAdminRecord, secretDigest } from "../_admin.js";
...
const WEB_THEMES = ["liquid-glass","minimalist","liquid-glass-solid","neumorphic"];
const ACCENT_PRESETS = {
  "Sky Blue": {hex:"#38bdf8",glow:"rgba(2,132,199,.28)"},
  "Teal": {hex:"#0d9488",glow:"rgba(17,94,89,.28)"},
  "Lilac": {hex:"#9d4edd",glow:"rgba(123,44,191,.28)"},
  "Moss Green": {hex:"#4d908e",glow:"rgba(39,76,74,.28)"},
  "Maroon": {hex:"#800020",glow:"rgba(92,0,23,.28)"},
  "Lavender": {hex:"#b79ced",glow:"rgba(144,122,214,.28)"},
  "Pink Sakura": {hex:"#ff70a6",glow:"rgba(255,71,126,.28)"},
  "Midnight": {hex:"#1d3557",glow:"rgba(11,19,41,.28)"},
  "Mint": {hex:"#2ec4b6",glow:"rgba(32,156,144,.28)"},
  "Charcoal": {hex:"#4a4e69",glow:"rgba(34,34,59,.28)"}
};
const DEFAULT_ACCENT = {name:"Lavender", ...ACCENT_PRESETS.Lavender};
...
function safeConfig(config, admins = [], toolConfig = null, registeredTools = null) {
  const tools = Array.isArray(registeredTools) ? registeredTools : getOtherTools();
  const configuredTools = Array.isArray(toolConfig) ? toolConfig : [];
  const toolMap = new Map(configuredTools.map(x => [String(x?.id), x]));
  return {
    maintenance: config?.maintenance === true,
    uiMode: config?.uiMode === "blur" ? "blur" : "liquid-glass",
    webTheme: WEB_THEMES.includes(config?.webTheme) ? config.webTheme : "liquid-glass",
    accent: (() => {
      const name=String(config?.accent?.name||"");
      const preset=ACCENT_PRESETS[name] ? {name,...ACCENT_PRESETS[name]} : DEFAULT_ACCENT;
      return preset;
    })(),
    font: {
    ...
    whatsappLink: String(config?.whatsappLink || "https://whatsapp.com/channel/0029Vb76SfPDzgTBlh5Tn70I"),
    otherTools: tools.map(tool => {
      const found = toolMap.get(tool.id);
      return { ...tool, enabled: found ? found.enabled === true : tool.defaultEnabled === true };
    })
  };
}

async function readOtherToolsConfig() {
  const value = await readKVJson(OTHER_TOOLS_KV_KEY);
...
async function readConfig() {
  try {
    ...
    if (String(error.message).includes("404")) {
      return { data: { maintenance:false, uiMode:"liquid-glass", webTheme:"liquid-glass", accent:DEFAULT_ACCENT, font:{family:"san-francisco",weight:600}, admins:[] }, sha:null };
    }
    throw error;
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if(!config || typeof config!=="object") {
      ...
      config = { maintenance:false, uiMode:"liquid-glass", webTheme:"liquid-glass", accent:DEFAULT_ACCENT, font:{family:"san-francisco",weight:600} };
      }
    }
    config = config || { maintenance:false, uiMode:"liquid-glass", webTheme:"liquid-glass", accent:DEFAULT_ACCENT, font:{family:"san-francisco",weight:600} };
    const toolConfig = await readOtherToolsConfig();

    if (req.method === "GET") {
      return res.status(200).json({ success:true, config:safeConfig(config, persistent.admins, toolConfig, registeredTools), storage:persistent.storage });
    }
    ...
    if (action === "addAdmin") {
      ...
      return res.status(200).json({
        ...
        admin: generated,
        config:safeConfig(config, admins, toolConfig),
    ...
    if (action === "deleteAdmin") {
      ...
      return res.status(200).json({ success:true, message:"Admin konfigurasi dihapus.", config:safeConfig(config, admins, toolConfig), storage:persistent.storage });
    }

    if (action === "otherTools") {
      const incoming = Array.isArray(body.tools) ? body.tools : [];
      const registered = await ensureOtherToolsLoaded();
      const next = registered.map(tool => {
        const found = incoming.find(x => String(x?.id) === tool.id);
        return { id: tool.id, enabled: found ? found.enabled === true : tool.defaultEnabled === true };
      });
      await writeKVJson(OTHER_TOOLS_KV_KEY, next);
      const savedTools = await readOtherToolsConfig();
      return res.status(200).json({success:true,message:"Other Tools diperbarui.",config:safeConfig(config,persistent.admins,savedTools),storage:"kv"});
    }
    ...
    if (action === "site") {
      ...
      if (body.webTheme !== undefined) config.webTheme = WEB_THEMES.includes(String(body.webTheme)) ? String(body.webTheme) : "liquid-glass";
      if (body.accent !== undefined) {
        const incoming=body.accent && typeof body.accent === "object" ? body.accent : {};
        const name=String(incoming.name||"");
        if (ACCENT_PRESETS[name]) config.accent={name,...ACCENT_PRESETS[name]};
      }
      if (body.font) {
      ...
      return res.status(200).json({success:true,message:written?"Konfigurasi berhasil disimpan dan disinkronkan ke GitHub.":"Konfigurasi berhasil disimpan ke KV. GitHub backup belum tersedia.",config:safeConfig(config,persistent.admins,await readOtherToolsConfig()),commit:written,storage:"kv"});
    }
