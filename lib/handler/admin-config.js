...
function safeConfig(config, admins = [], toolConfig = null) {
  ...
  return {
    ...
    otherTools: getOtherTools().map(tool => {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      return res.status(200).json({ success:true, config:safeConfig(config, persistent.admins, toolConfig), storage:persistent.storage });
    }
    ...
    if (action === "otherTools") {
      ...
      const registered = getOtherTools();
