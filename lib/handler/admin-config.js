...
function safeConfig(config, admins = []) {
  return {
    ...
    otherTools: DEFAULT_TOOLS.map(tool => { const found = Array.isArray(config?.otherTools) ? config.otherTools.find(x => String(x?.id) === tool.id) : null; return {...tool, enabled: found ? found.enabled === true : tool.defaultEnabled === true}; })
  };
}
