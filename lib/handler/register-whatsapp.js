...
import { getConnectedMainGateway } from '../_whatsapp.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const ts = await verifyTurnstile(turnstileToken, req);
    if (!ts.success) return res.status(403).json({ success:false, message:ts.message, code:'TURNSTILE_REQUIRED' });
    ...
    const phone = String(gateway.ownerPhone || '').replace(/\D/g, '');
