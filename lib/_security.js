...
function clientFingerprint(req) {
  // NOTE: intentionally NOT keyed on IP address. Behind Cloudflare (and on
  // mobile/cellular networks) the client's observed IP can change between
  // the challenge/verify request and a later authenticated request (edge
  // colo routing, carrier NAT rotation, etc). Binding the fingerprint to IP
  // caused legitimate users to be stuck in a permanent
  // "Verifikasi keamanan diperlukan" [HTTP 403] loop, since every fresh
  // token would already mismatch the very next request. User-agent +
  // accept-language is stable for the life of a browser tab/session and is
  // enough to keep this a lightweight bot/replay check rather than a hard
  // per-request device lock.
...
export function verifySecurityToken(token, req = null) {
  ...
  if (payload.type !== "browser-security" || !payload.jti || !payload.fp) return { ok: false, reason: "bad_payload" };
  if (!Number.isFinite(Number(payload.exp)) || Date.now() > Number(payload.exp)) return { ok: false, reason: "expired" };
  // NOTE: the fingerprint is intentionally NOT re-checked here. It's already
  // validated once, tightly, during the short-lived (5 minute) challenge->verify
  // exchange in verifySecurityChallenge() below. Re-checking it again on every
  // later request against a token that can live for up to 12 hours caused
  // legitimate users to get permanently stuck behind "Verifikasi keamanan
  // diperlukan" [HTTP 403] whenever the browser's reported headers drifted even
  // slightly over that window (Accept-Language reordering, in-app browser mode
  // switches, etc). Signature + expiry are enough to trust a previously-issued
  // token; the proof-of-work challenge already did the real bot-filtering work.
...
export async function requireSecurity(req, res) {
  ...
  if (!token) {
    return res.status(403).json({ success: false, code: "SECURITY_VERIFICATION_REQUIRED", reason: "missing_token", message: "Verifikasi keamanan diperlukan. Silakan selesaikan verifikasi browser terlebih dahulu." });
  }
  const check = verifySecurityToken(token, req);
  if (!check.ok) {
    return res.status(403).json({ success: false, code: "SECURITY_VERIFICATION_REQUIRED", reason: check.reason, message: "Verifikasi keamanan diperlukan. Silakan selesaikan verifikasi browser terlebih dahulu." });
  }
...
export async function rateLimit(req, bucket, limit = 10, windowSeconds = 600) {
  ...
  await store.put(key, JSON.stringify({ count, resetAt: current.resetAt }), { expirationTtl: Math.max(60, Math.ceil((Number(current.resetAt) - now) / 1000)) });
